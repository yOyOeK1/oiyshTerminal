...
class apVBrain2{
	...
	makeNewPID(){
		...
		this.pid.setGains( this.config['P'],this.config['I'],this.config['D'] );
		//this.pid.setItermLimit(-this.config['zeroMaxOffset'],this.config['zeroMaxOffset']);
    this.pid.setItermLimit(-0.3,0.3);
    this.msgCount = 0;
	}
  ...
  useBrain = false;
    ...
    var deltaSS = this.avgItKalman("ap3Delta", this.config['avgSmoothing']*5 );
    ...
    if( this.useBrain == false ){
        ...
        if( this.msgCount> 10 ){
          ...
          bRes = this.brain.run([deltaS]);
        }
        this.tLast = til;
    }else{
      if( Math.abs(deltaSS) > 3 )
        this.brain.train([[
          parseFloat( deltaS ),
          parseFloat( til  )
        ]]);
      ...
      bRes = this.brain.run([deltaS]);
      forcast = this.brain.forecast([deltaS],1)[0];
      this.tillerBy( this.tLast-bRes );
      this.tLast = bRes;
    ...
    //console.log(forcast);
		var debVals = {
			...
			//'target': this.target,
			'pRes': til,
			'useBrain': (this.useBrain? 'yes':'no')+" res:"+bRes.toFixed(5)+" for:"+forcast.toFixed(5),
			'tillerPos': this.tillerPos,
			'delta': this.delta,
			'deltaS': deltaS.toFixed(2)+" -> "+deltaSS.toFixed(2),
			'offset':this.offset
